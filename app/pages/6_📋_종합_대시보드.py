...
# -----------------------------------------------------------------------------
# 7. PR-29 — 본선 직전 강화 패키지 요약 (PR-17/18/9F/28)
# -----------------------------------------------------------------------------
st.divider()
st.subheader("🌟 본선 직전 최종 강화 — 신뢰성 + 접근성 + UX")

enh_c1, enh_c2 = st.columns(2)
with enh_c1:
    st.markdown(
        """
        ### 📐 PR-17 · Cpk 공정 능력 지수
        AIAG 자동차 SPC 표준 — 통계 전문가 없어도 **1개 숫자(Cpk)** 로 의사결정.
        - 🔴 < 1.00 부적합 · 🟡 1.00~1.33 개선 · 🟢 1.33~1.67 양호 · 🌟 ≥ 1.67 우수
        - **P5 SPC/Pareto** 페이지 ④ 섹션에 게이지 + 카드 + USL/LSL 슬라이더

        ### 🎨 PR-18 · Okabe-Ito 색맹 친화
        WCAG 1.4.1 준수 — 색맹 사용자도 형태로 구분.
        - ● 정상 #009E73 · ▲ 경고 #E69F00 · ■ 위험 #D55E00
        - 색상 + 기호 + 한글 라벨 + 두꺼운 테두리 **4중 중복 전달**
        """
    )
with enh_c2:
    st.markdown(
        """
        ### 🎭 PR-9F · 시연 시나리오 토글
        발표 시 페르소나 흐름 자동 강조 + 추천 샘플 ⭐ 마킹.
        - 🟢 NORMAL — 정상 운영 (proba < 0.30)
        - 🟡 WARN — 경고 신호 (0.30 ≤ proba < 0.50)
        - 🔴 ANOMALY — 명백한 이상 (proba ≥ 0.50, 9/51 추천)

        ### 🔧 PR-28 · Mode 핫픽스 3중 방어선
        - **L1**: widget+persistent 양방향 sync
        - **L2**: ensure_demo_state에서 URL 즉시 push
        - **L3**: JS MutationObserver + click interceptor
        """
    )

cpk_k1, cpk_k2, cpk_k3, cpk_k4 = st.columns(4)
cpk_k1.metric("단위 테스트", "65/65", "전 PR 회귀 PASS")
cpk_k2.metric("머지 PR (P-F)", "11건", "#90~102")
cpk_k3.metric("접근성", "WCAG AA", "Okabe-Ito 통일")
cpk_k4.metric("통계 표준", "3종", "SPC + Pareto + Cpk")

st.success(
    "✅ **본선 제출 준비 완료** — 신뢰성(테스트 65) + 접근성(색맹) + "
    "통계 표준(Cpk) + UX(시나리오) 4축 강화."
)
