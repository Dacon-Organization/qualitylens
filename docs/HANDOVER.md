...
## 0. 한 줄 현황
**P-F 본선 직전 필수 7건 모두 머지 완료!** PR-21/E1R/23/22/26/24/E5 (#90~96). 회귀 20/20 PASS. **사용자 검증 + 대회 제출 대기 단계.** 선택 PR-17/18/9F는 시간 여유 시.
...
1. **사용자 직접 검증** — Streamlit Cloud Reboot 후 6 시나리오 (모드 토글 / 페이지 이동 / 업로드 흐름 / F5)
2. **사용자 직접 작업** — slides.html → 브라우저 Ctrl+P → PDF 16장 출력 + demo_video_guide.md 따라 OBS 녹화 → YouTube Unlisted 업로드
3. **선택 PR-17/18/9F** — 시간 여유 시 Cpk 게이지(40분) / 색맹 UI(35분) / 시나리오 토글(30분)
...
| PR-E5 | 발표 16:9 슬라이드 12 → 16장 | ✅ 머지(#96) |
| PR-17/18/9F | 선택 (Cpk / 색맹 / 시나리오) | ⏳ 시간 여유 시 |
| 사용자 작업 | reboot 검증 + PDF 16장 출력 + 영상 녹화 | ⏳ 대회 제출 |
