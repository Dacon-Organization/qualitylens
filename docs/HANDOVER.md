...
## 0. 한 줄 현황
**P-F 본선 직전 종합 패키지 진행 중** — P-B 11건 머지, P-F PR-21(#90)+PR-E1R(#91) 머지, **PR-23 Onboarding 7슬라이드 완료** → 다음 PR-22 시각화 풀 확장.
...
- 2026-05-22 13:15: **PR-23** — onboarding.py 4→7슬라이드 확장. SVG inline 시각화 3장 신규 (demo flow / SHAP example / action flow). 비전문가용 자세한 해설 + 실제 동작 흐름 (사용자 처방 반영).
...
1. **PR-23 commit + push + PR + auto merge** (브랜치 `claude/p-f-pr23-onboarding-7slides`)
2. **PR-22 시각화 풀 확장** — viz_advanced.py 신규 모듈, 5페이지 P0/P1 차트 추가 (재생/점멸/SPC mini/violin/heatmap/box/ROI/confusion/누적)
3. **PR-26 인앱 UX** — sensor_dictionary.csv + action_rules.py + CTA switch_page + 빈 화면 방어
...
| PR-E1R | 발표 16:9 재작업 (320×180mm) + KPI/Verdict 패턴 | ✅ 머지(#91) |
| **PR-23** | Onboarding 7슬라이드 + 데이터 프로파일링 (SVG 3장) | ✅ 로컬 완료, commit/push 대기 |
| PR-22 | 시각화 풀 확장 (Step 4~7) | 🚧 다음 |
